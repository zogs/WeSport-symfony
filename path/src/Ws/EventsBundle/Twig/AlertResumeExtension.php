    ...
    public function generateContext(Alert $alert)
    {
        ...
        if($search->hasLevel()){
            ...
            foreach ($search->getLevelNames() as $key => $level) {
                $tx .= $this->translator->trans('event.level.'.$level,array(),'events').' ';
            }
            $tx .= '</strong></span>';                        
        }        
        ...
        if($search->hasDayOfWeek()){
            ...
            foreach ($search->getDayOfWeek() as $key => $day) {
                $tx .= $this->translator->trans('days.day.'.lcfirst($day),array(),'utils').' ';
            }
            $tx .= '</strong></span>';
        }
        
        // aprés 10h et avant 18h
        if($search->hasTimeStart())  $tx .= '<span> aprés <strong>'.$search->getTimeStart()->format('H\hi').'</strong>';
        if($search->hasTimeStart() && $search->hasTimeEnd()) $tx .= ' et';
        if($search->hasTimeEnd()) $tx .= ' avant <strong>'.$search->getTimeEnd()->format('H\hi').'</strong></span>';        
